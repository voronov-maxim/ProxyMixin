...
namespace ProxyMixin
{
    ...
    public sealed class ArrayCopyProxyMapper<T> : ProxyMapper<T>
    {
        ...
        protected override void Emit(ILGenerator il, FieldInfo fieldInfo)
        {
            if (fieldInfo.FieldType.IsArray)
            {
                ...
                MethodInfo fakeCopyArray;
                if (elementType.IsPrimitive)
                    fakeCopyArray = ((Func<int[], int[], int[]>)CopyPrimitiveArray<int>).Method;
                else
                    fakeCopyArray = ((Func<Object[], Object[], Object[]>)CopyObjectArray<Object>).Method;
                MethodInfo copyArray = fakeCopyArray.GetGenericMethodDefinition().MakeGenericMethod(elementType);
                ...
                il.Emit(OpCodes.Ldarg_1);
                il.Emit(OpCodes.Ldfld, fieldInfo);
                il.Emit(OpCodes.Call, copyArray);
